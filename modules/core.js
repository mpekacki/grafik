...
function getSummary(dateFrom, dateTo, cb){
	db.query('WITH total AS (WITH summary AS (WITH comments AS (SELECT "id", "name", "nf_id", "date", "permanent" FROM "Judges", "Stories" WHERE "date" BETWEEN $1 AND $2 AND "active" = true) SELECT comments."name", sum(CASE WHEN EXTRACT (DOW FROM comments."date") = "day_of_week" THEN 1 ELSE 0 END) > 0 AS "on_duty", COALESCE("comment_count", 0) AS "comment_count" FROM comments LEFT JOIN "StoriesJudgesComments" AS p ON p."StoryId" = comments."nf_id" AND p."JudgeId" = comments."id" INNER JOIN "Duties" ON "Duties"."JudgeId" = comments."id" GROUP BY comments."name", "comment_count", comments."nf_id", comments."permanent" ORDER BY comments."permanent" DESC, comments."name" ASC) SELECT "name", sum(CASE WHEN "on_duty" AND "comment_count" > 0 THEN 1 ELSE 0 END) AS "quota_done", sum(CASE WHEN "on_duty" THEN 1 ELSE 0 END) AS "quota_total", sum(CASE WHEN NOT "on_duty" AND "comment_count" > 0 THEN 1 ELSE 0 END) AS "hobby_done", sum(CASE WHEN NOT "on_duty" THEN 1 ELSE 0 END) AS "hobby_total" FROM summary GROUP BY "name" ORDER BY "name") SELECT "name", "quota_done", "quota_total", CASE WHEN "quota_total" = 0 THEN 100 ELSE round("quota_done"::float / "quota_total" * 100) END AS "quota_percent", "hobby_done", "hobby_total", CASE WHEN "hobby_total" = 0 THEN 100 ELSE round("hobby_done"::float / "hobby_total" * 100) END AS "hobby_percent", "quota_done" + "hobby_done" AS "all_done", "quota_total" + "hobby_total" AS "all_total", CASE WHEN ("quota_total" + "hobby_total") = 0 THEN 100 ELSE round(("quota_done" + "hobby_done")::float / ("quota_total" + "hobby_total") * 100) END AS "all_percent" FROM total;',
